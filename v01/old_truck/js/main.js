(function($) {
    $(document).ready(function() {
        ...
        */
        ...
        $('.left-sidebar, .right-sidebar').css('width', calculated_width+'px');
        //on resize
        $(window).on('resize orientationChange', function(event) {
            container_width = $('.container').width();
            calculated_width = ($(window).width()/2)-(container_width/2);
            $('.left-sidebar, .right-sidebar').css('width', calculated_width+'px');
            

        });
         
        // $('.left-sidebar').css('height', c_height+'px');
        var imgsArray = [
        ...
        function setLeftImage(left, top, imageName){
            $(".left-sidebar").append("<a href='#'><img index='"+img_index+"' src='img/trucks_light/" + imageName + ".png'></a>");
            $(".left-sidebar a").last().css({"top": top + "px", "left": left + "px"});
        }
        
        function setRightImage(right, top, imageName){
            $(".right-sidebar").append("<a href='#'><img index='"+img_index+"' src='img/trucks_light/" + imageName + ".png'></a>");
            $(".right-sidebar a").last().css({"top": top + "px", "right": right + "px"});
        }    

        //Setting image on both sidebar
        // if(userAgent.width() > 1024){
        var top=0;
        var quarterWidth = Math.floor(leftWidth/4);
        while( top<leftHeight){
            var pos = getRandom(0, quarterWidth);

            var img_index = generateRandomForArray();
            var imageName = imgsArray[img_index];
            setLeftImage(pos, top, imageName);

            img_index = generateRandomForArray();
            imageName = imgsArray[img_index];
            setRightImage(pos, top, imageName);

            if(pos <= quarterWidth/2){
            ...
            $('.left-sidebar a img, .right-sidebar a img').mouseenter(function(){
                ...
                $(this).attr('src', 'img/trucks/' + imgsArray[img_index] + '.png')
            });

            $('.left-sidebar a img, .right-sidebar a img').mouseleave(function(){
                var img_index = ($(this).attr('index'));
                $(this).attr('src', 'img/trucks_light/' + imgsArray[img_index] + '.png')
            });
        //}
